	...
	if (isset($_POST["changeresume"])) {
		$resumeFileName = $_FILES["fileToUpload"]["name"];
		$resumeTmpName = $_FILES["fileToUpload"]["tmp_name"];

		// Check if a file was uploaded
		if (!empty($resumeFileName) && is_uploaded_file($resumeTmpName)) {
			$allowedFormats = ["pdf", "docx"]; // Add more allowed formats as needed
			...
			if (in_array($fileExtension, $allowedFormats)) {
				...
				$newResumeFileName = uniqid() . "." . $fileExtension;
				$targetDir = "../photoJ/";
				$targetPath = $targetDir . $newResumeFileName;

				// Move the uploaded file to the target directory
				if (move_uploaded_file($resumeTmpName, $targetPath)) {
					// Update the resume file name in the database
					$sql = "UPDATE employee SET resume = ? WHERE mail = ?";
					$stmt = mysqli_prepare($conn, $sql);
					mysqli_stmt_bind_param($stmt, "ss", $newResumeFileName, $mail);
				...
				echo "<script type=\"text/javascript\">alert(\"Invalid file format. Allowed formats: PDF, DOCX.\");</script>";
			}
		} else {
