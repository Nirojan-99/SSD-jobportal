...
if (isset($_POST["changeemail"])) {
	$newEmail = htmlspecialchars($_POST['mail'], ENT_QUOTES, 'UTF-8');
	...
	// Check if the new email address is valid and not already in use
	$sql = "SELECT COUNT(*) as count FROM $userType WHERE mail = ?";
	...
	if ($count > 0) {
		...
		echo "<script type='text/javascript'>alert(\"Email address is already in use.\");</script>";
	} else {
		// Use prepared statements for the database update to prevent SQL injection
		$sql = "UPDATE $userType SET mail = ? WHERE mail = ?";
		$stmt = mysqli_prepare($conn, $sql);
		mysqli_stmt_bind_param($stmt, "ss", $newEmail, $mail);

		if (mysqli_stmt_execute($stmt)) {
			if (mysqli_affected_rows($conn) > 0) {
				// Email address updated successfully
				$_SESSION['mailP'] = $newEmail;
				if ($validation == 1) {
				...
				echo "<script type=\"text/javascript\">alert(\"Unable to update email address.\");</script>";
			}
		} else {
			echo "<script type=\"text/javascript\">alert(\"Error updating email address.\");</script>";
		}
	}
}
