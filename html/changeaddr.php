	...
	if (isset($_POST["changeaddress"])) {
		$newAddress = htmlspecialchars($_POST['addr'], ENT_QUOTES, 'UTF-8');
		...
		$sql = "UPDATE $userType SET address = ? WHERE mail = ?";
		$stmt = mysqli_prepare($conn, $sql);
		mysqli_stmt_bind_param($stmt, "ss", $newAddress, $mail);
		...
		if (mysqli_stmt_execute($stmt)) {
			if (mysqli_affected_rows($conn) > 0) {
				...
				echo "<script type=\"text/javascript\">alert(\"Unable to update address.\");</script>";
			}
		} else {
