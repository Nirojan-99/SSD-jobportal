...
<body id="total">
	<div class="backL" id="backL">
		...
		if (isset($_POST["deleteC"])) {
			...
			// Check if the employer account with the provided email exists
			...
			if (mysqli_num_rows($result) > 0) {
				// Employer account found, proceed with deletion
				$sql = "DELETE FROM employer WHERE email = ?";
				$stmt = mysqli_prepare($conn, $sql);
				mysqli_stmt_bind_param($stmt, "s", $mail);
				mysqli_stmt_execute($stmt);
                ...
                document.getElementById('msg').innerHTML=\"Deleted successfully!\"
					...
					echo "<script type='text/javascript'>
                document.getElementById('msg').innerHTML=\"Error deleting the employer account.\"
            </script>";
				}
			} else {
				echo "<script type='text/javascript'>
            document.getElementById('msg').innerHTML=\"Employer account with the provided email not found.\"
        </script>";
			}
		}
