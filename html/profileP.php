	...
	$searchMail = "%" . $mail . "%";
	mysqli_stmt_bind_param($stmt, "s", $searchMail);
	...
	$sql = "SELECT * FROM favorite_job WHERE employee_id LIKE ?";
	$stmt = mysqli_prepare($conn, $sql);
	mysqli_stmt_bind_param($stmt, "s", $id);
	mysqli_stmt_execute($stmt);
	$result = mysqli_stmt_get_result($stmt);
	$favjob = mysqli_num_rows($result);

	$sql = "SELECT * FROM applied_job WHERE employee_id LIKE ?";
	$stmt = mysqli_prepare($conn, $sql);
	mysqli_stmt_bind_param($stmt, "s", $id);
	mysqli_stmt_execute($stmt);
	$result = mysqli_stmt_get_result($stmt);
	$applyjob = mysqli_num_rows($result);

	$sql = "SELECT * FROM interview WHERE employee_id LIKE ?";
	$stmt = mysqli_prepare($conn, $sql);
	mysqli_stmt_bind_param($stmt, "s", $id);
	mysqli_stmt_execute($stmt);
	$result = mysqli_stmt_get_result($stmt);
	$approvedjob = mysqli_num_rows($result);
	...
	$sql = "SELECT * FROM job ORDER BY posted_date DESC";
	...
	if (isset($_POST["removephoto"])) {
		$sql = "UPDATE employee SET profile_image = 'user1.png' WHERE employee_id LIKE ?";
		$stmt = mysqli_prepare($conn, $sql);
		mysqli_stmt_bind_param($stmt, "s", $id);
		mysqli_stmt_execute($stmt);
		...
		<div class="backL" id="backL">
			...
			<fieldset class="containP">
				<fieldset class="profileE">
					...
					<ul>
						<hr>
						<li>User Name : <?php echo $name ?> <a href="changeuser.php" class="EP">Edit</a></li>
						<hr>
						<li>Father Name : <?php echo $fname ?> </li>
						<hr>
						<li>Gender : <?php echo $gender ?> </li>
						<hr>
						<li>date of birth : <?php echo $dob ?> </li>
						<hr>
						<li>User Email : <?php echo $mail ?> <a href="changemail.php" class="EP">Edit</a></li>
						<hr>
						<li>Mobile No : <?php echo $mobile ?> <a href="changemob.php" class="EP">Edit</a></li>
						<hr>
						<li>User Address : <?php echo $addr ?> <a href="changeaddr.php" class="EP">Edit</a></li>
						<hr>
				...
				<fieldset class="profileE">
					...
					<ul>
						<hr>
						<li>No of Applied Jobs : <?php echo $applyjob ?></li>
						<hr>
						<li>No of Jobs in Favorite : <?php echo $favjob ?></li>
						<hr>
						<li>No of Interviews : <?php echo $approvedjob ?></li>
						<hr>
					</ul>
				</fieldset>
