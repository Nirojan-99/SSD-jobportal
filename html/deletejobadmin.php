...
<body id="total">
	<div class="backL" id="backL">
		...
		if (isset($_POST["deleteC"])) {
			...
			// Validate and sanitize the email and job name inputs
			$maild = filter_var($maild, FILTER_SANITIZE_EMAIL);
			$job = mysqli_real_escape_string($conn, $job);

			// Check if the job exists for the given employer's email
			$sql = "SELECT * FROM job WHERE job_name = ? AND company_id = (SELECT company_id FROM employer WHERE email = ?)";
			$stmt = mysqli_prepare($conn, $sql);
			mysqli_stmt_bind_param($stmt, "ss", $job, $maild);
			mysqli_stmt_execute($stmt);
			$result = mysqli_stmt_get_result($stmt);

			if (mysqli_num_rows($result) > 0) {
				// Job found, proceed with deletion
				$sql = "DELETE FROM job WHERE job_name = ? AND company_id = (SELECT company_id FROM employer WHERE email = ?)";
				$stmt = mysqli_prepare($conn, $sql);
				mysqli_stmt_bind_param($stmt, "ss", $job, $maild);
				mysqli_stmt_execute($stmt);
                ...
                document.getElementById('msg').innerHTML=\"Deleted successfully!\"
					...
					echo "<script type='text/javascript'>
                document.getElementById('msg').innerHTML=\"Error deleting the job.\"
            </script>";
				}
			} else {
				echo "<script type='text/javascript'>
            document.getElementById('msg').innerHTML=\"Job name not found for the given email address.\"
        </script>";
			}
		}
