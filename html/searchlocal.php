		...
		<div>
			...
			<aside>
				...
				<fieldset class="jobviewS">
					<?php
					//
					if (isset($_POST["searchL"]) || isset($_POST["submitF"])) {
						...
						if (isset($_POST["searchL"])) {
							...
							try {
								...
								$sql = "SELECT * 
										FROM job a
										JOIN employer b ON a.company_id = b.company_id
										WHERE job_name LIKE ?";
						...
						if (isset($_POST["submitF"])) {
							...
							try {
								...
								// Start building the SQL query with placeholders
								$sql = "SELECT * 
										FROM job a
										JOIN employer b ON a.company_id = b.company_id
										WHERE ";
								...
								if (!empty($_POST['Experience'])) {
									...
									if ($experience == "Fresher") {
										$conditions[] = "a.experience = 0";
									} else if ($experience == "1-3 Years") {
										$conditions[] = "a.experience >= 1 AND a.experience <= 3";
									} else if ($experience == "3-5 Years") {
										$conditions[] = "a.experience > 3 AND a.experience <= 5";
									} else {
										$conditions[] = "a.experience >= 6";
									}
								}
								...
								if (!empty($_POST['Salary'])) {
									...
									if ($salary == "15000") {
										$conditions[] = "a.salary < 15000";
									} else if ($salary == "25000") {
										$conditions[] = "a.salary >= 15000 AND a.salary < 25000";
									} else if ($salary == "50000") {
										$conditions[] = "a.salary >= 25000 AND a.salary < 50000";
									} else {
										$conditions[] = "a.salary >= 50000";
									}
								}

								if (!empty($_POST['Companies'])) {
									$Companies = $_POST['Companies'];
									$conditions[] = "b.company_name LIKE ?";
								}
								...
								// Use prepared statement for the filtered search query
								$stmt = mysqli_prepare($conn, $sql);

								// Bind parameters
								$paramTypes = str_repeat('s', count($conditions));
								$paramValues = array($gender, $type, $Companies, $location);
								$paramValues = array_merge($paramValues, array_fill(0, count($conditions) - count($paramValues), $searchParam));
