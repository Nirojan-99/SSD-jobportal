	...
	if (isset($_POST["changemob"])) {
		$newMobileNumber = htmlspecialchars($_POST['num'], ENT_QUOTES, 'UTF-8');
		...
		$sql = "UPDATE $userType SET mobile_number = ? WHERE mail = ?";
		$stmt = mysqli_prepare($conn, $sql);
		mysqli_stmt_bind_param($stmt, "ss", $newMobileNumber, $mail);
		...
		if (mysqli_stmt_execute($stmt)) {
			if (mysqli_affected_rows($conn) > 0) {
				...
				echo "<script type=\"text/javascript\">alert(\"Unable to update mobile number.\");</script>";
			}
		} else {
			echo "<script type=\"text/javascript\">alert(\"Error updating mobile number.\");</script>";
		}
	}
