		...
		if (isset($_POST["otp"])) {
			...
			if ($otpcode != $otpcodeS) {
				...
				if ($user == "employer") {
					...
					$sql = "INSERT INTO employee(employee_name, father_name, gender, mail, password, date_of_birth, mobile_number, address, profile_image) 
							VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'user1.png')";
					$stmt = mysqli_prepare($conn, $sql);
					mysqli_stmt_bind_param($stmt, "ssssssss", $name, $fname, $gender, $mail, $pass, $DOB, $num, $addr);
					mysqli_stmt_execute($stmt);
