...
<body id="total">
	<div class="backL" id="backL">
		...
		<fieldset class="hdd">
			<p class="hd">INTERVIEW DETAILS</p>
		</fieldset>
		<!-- noti section -->
		<?php


		if (isset($_POST["jobid"]) && isset($_POST["jobname"]) && isset($_POST["empid"]) && isset($_POST["name"])) {
			$jobid = htmlspecialchars($_POST['jobid'], ENT_QUOTES, 'UTF-8');
			$jobname = htmlspecialchars($_POST['jobname'], ENT_QUOTES, 'UTF-8');
			$empid =  htmlspecialchars($_POST['empid'], ENT_QUOTES, 'UTF-8');
			$name =  htmlspecialchars($_POST['name'], ENT_QUOTES, 'UTF-8');

			// Output the form for job applicant approval
			echo '<fieldset class="approve">
        <form action="../php/index.php" method="post">
            <label for="employee">APPLICANT NAME</label>
            <input name="employee" value="' . $name . '" readonly><br>
            
            <label for="job">JOB NAME</label>
            <input name="job" value="' . $jobname . '" readonly><br>
            ...
            <textarea name="venue" required id="note"></textarea><br>
            
            <label for="msg">MESSAGE TO APPLICANT</label>
            <textarea name="msg" required id="note"></textarea><br>
            
            <center>
                <input type="submit" value="Approve" id="apr" name="approve">
            </center>
        </form>
    </fieldset>';
		}
