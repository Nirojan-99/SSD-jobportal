	...
	<body id="total">
		<div class="backL" id="backL">
			...
			if (isset($_POST["changePass"])) {
				$newPassword = password_hash(htmlspecialchars($_POST['passNew'], ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT);
				$currentPassword = htmlspecialchars($_POST['passE'], ENT_QUOTES, 'UTF-8');

				// Determine whether the user is an employee or an employer based on $validation
				if ($validation == 1) {
					$userType = "employee";
				} elseif ($validation == 2) {
					$userType = "employer";
				...
				$sql = "SELECT password FROM $userType WHERE mail = ?";
				...
				if (mysqli_stmt_execute($stmt)) {
					...
					if (password_verify($currentPassword, $row['password'])) { 
						...
						$sql = "UPDATE $userType SET password = ? WHERE mail = ?";
						$stmt = mysqli_prepare($conn, $sql);
						mysqli_stmt_bind_param($stmt, "ss", $hashedPassword, $mail);

						if (mysqli_stmt_execute($stmt)) {
							// Password changed successfully
							$_SESSION['passP'] = $newPassword;
							if ($validation == 1) {
							...
							echo "<script type='text/javascript'>alert(\"Error updating the password.\");</script>";
						}
					} else {
						echo "<script type='text/javascript'>document.getElementById('msg').innerHTML=\"Incorrect current password !!\";</script>";
					}
				} else {
					echo "<script type='text/javascript'>alert(\"Error retrieving the current password.\");</script>";
				}
			}
