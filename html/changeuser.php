...
<body id="total">
	<div class="backL" id="backL">
		...
		if (isset($_POST["changeuser"])) {
			$pass = $_POST['passNew'];
			$newUser = htmlspecialchars($_POST['Nuser'], ENT_QUOTES, 'UTF-8');

			// Function to verify password and update the username
			function verifyPasswordAndUpdateUser($conn, $tableName, $mail, $pass, $newUser)
			{
				$sql = "SELECT password FROM $tableName WHERE " . ($tableName == 'employee' ? 'mail' : 'email') . " = ?";
				...
				if ($row = mysqli_fetch_assoc($result)) {
					...
					if (password_verify($pass, $hashedPassword)) {
						$updateSql = "UPDATE $tableName SET " . ($tableName == 'employee' ? 'employee_name' : 'company_name') . " = ? WHERE " . ($tableName == 'employee' ? 'mail' : 'email') . " = ?";
						$updateStmt = mysqli_prepare($conn, $updateSql);
						mysqli_stmt_bind_param($updateStmt, "ss", $newUser, $mail);
			...
			if (verifyPasswordAndUpdateUser($conn, 'employee', $mail, $pass, $newUser) || verifyPasswordAndUpdateUser($conn, 'employer', $mail, $pass, $newUser)) {
