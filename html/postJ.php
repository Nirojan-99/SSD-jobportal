...
<body id="total">
	<div class="backL" id="backL">
		...
		if (isset($_POST["submit"])) {
			...
			$job = htmlspecialchars($_POST['jname'], ENT_QUOTES, 'UTF-8');
			$location = htmlspecialchars($_POST['location'], ENT_QUOTES, 'UTF-8');
			$ddate = htmlspecialchars($_POST['dudedate'], ENT_QUOTES, 'UTF-8');
			$salary = htmlspecialchars($_POST['salary'], ENT_QUOTES, 'UTF-8');
			$qlty = htmlspecialchars($_POST['qualification'], ENT_QUOTES, 'UTF-8');
			$gen = htmlspecialchars($_POST['gen'], ENT_QUOTES, 'UTF-8');
			$xpe = htmlspecialchars($_POST['experience'], ENT_QUOTES, 'UTF-8');
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$stmt = mysqli_prepare($conn, $sql);
			mysqli_stmt_bind_param($stmt, "ssssssssss", $job, $company, $today, $ddate, $salary, $location, $qlty, $type, $xpe, $gen);
			mysqli_stmt_execute($stmt);
